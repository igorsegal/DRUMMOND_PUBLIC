...
FIX1:
ND02 теперь принимает не только 28 FX-пар, но и инструменты типа XAUUSD,
XAGUSD, BTCUSD, если ровно одна из сторон инструмента входит в
AUD/CAD/CHF/EUR/GBP/JPY/NZD/USD.

Для таких инструментов используется экспериментальный ONE_SIDED режим:
например XAUUSD -> внешний сигнал строится по USD strength, затем
ориентируется в сторону цены XAUUSD (сильный USD = отрицательное внешнее
давление на XAUUSD). Это отдельная экспериментальная ветка и не является
тем же самым двухвалютным сигналом, что для EURUSD.
...
- для FX-пар использует полный двухвалютный сигнал;
- для XAUUSD/XAGUSD/BTCUSD и аналогичных целей использует ONE_SIDED сигнал
  по единственной известной валюте из восьми.
